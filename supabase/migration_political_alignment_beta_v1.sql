...
BEGIN
  ...
  IF v_cur = 'CENTRAL' THEN
    IF p_score >= 360 THEN RETURN 'PIONEER'; END IF;
    IF p_score <= -360 THEN RETURN 'GUARDIAN'; END IF;
    RETURN 'CENTRAL';
  END IF;
  IF v_cur = 'PIONEER' THEN
    IF p_score <= 160 THEN
      IF p_last_changed IS NOT NULL AND p_batch_at IS NOT NULL
         AND (p_batch_at - p_last_changed) < interval '48 hours' THEN
        RETURN 'PIONEER';
      END IF;
      RETURN 'CENTRAL';
    END IF;
