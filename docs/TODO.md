...
- [x] 로그인 회원이 본인 현재/최근 제재와 이의제기 상태·결과를 확인
- [x] 허용된 경우 기존 `POST /api/me/sanctions/appeals`로 제출
- [x] Guest는 사용 불가. 강제 로그인 이동 없음
- [x] 제재 생성/사다리/판정/관리자 API 미변경
- [ ] Production Chrome 실회원 화면 확인 (이번 작업 아님)

## ✅ 2026-09-05 — ADMIN/OWNER 공식글 최소 운영 경로

- [x] ADMIN/OWNER 공식글 작성·수정·종료 API/UI
- [x] 일반회원 위조 방지. 운영 공식글 XP/업적 미반영
- [x] commit `6600de9`
- [ ] 공식글 Production 배지 Chrome 검증 (보류)

## ⏸️ 2026-09-05 — 기존 보류 유지

- [ ] 첫 방문 안내 Chrome 검증
- [ ] 재로그인 후 첫 방문 안내 반복 여부
- [ ] 회원 권리침해 실제 제출
- [ ] 비회원 권리침해 안내
- [ ] 공식글 Production 배지 검증
...
## ⏸️ 2026-08-30 — 세션 종료 보류 (다음 작업자)

현재 Git: `master` = `origin/master` = `b2d3682`. 게스트 읽기 전용까지 push 완료. 오늘 추가 개발사항 Production / Railway / Supabase 운영 반영 없음.

- [ ] 네이버 로그인: 개발자센터 재검수 결과 대기. 승인/반려에 따라 대응
- [ ] 한국 서버 이전: 보류. Railway 유지. 추후 NAVER Cloud. 새 서버 완전 준비 후 DNS. Railway는 되돌리기용으로 잠시 유지. 단계별 진행
- [ ] 가입 정치정보/영토공개 동의: 코드 완료. Production 동의 칸 2개 apply 완료. 새 코드 운영 반영은 한국 서버 이전 후. 기존 회원 동의 화면·신규 Google/Kakao/Naver 가입 실제 확인
- [ ] 권리침해 강화: Production SQL 미적용 → 한국 서버 이전 후. 브라우저 접수→첨부→운영자 검토→반려/처리. 비회원 일반 신고 불가. 비회원 권리침해는 본인확인 수단 준비 전 실접수 차단. 이메일 연결 보류. 정식 건 5년 보관 최종 검토
- [ ] 첫 방문 안내: Production SQL 미적용 → 한국 서버 이전 후. 신규 소셜가입 1회·중앙광장 진입·모바일 확인
- [ ] 공식 게시글: Production `is_official` SQL 미적용 → 한국 서버 이전 후. 기존 운영자 UUID로 9편 등록. 가짜 회원/댓글/반응 금지
- [ ] 게스트 읽기 전용: 한국 서버 이전 후 브라우저 최종 확인
- [ ] Daily Issue: 실제 AI 연결 보류(현재 AI 수정은 진짜 AI 아님). 04:30 자동생성 ON 보류. Production 저장구조 최종 확인. 승인 전 자동공개 금지
- [ ] 법적 문서: 사이트 미연결. 한국 서버 이전 후 서버/국외처리·사업자/담당자/시행일·권리침해 최종 상태 반영 후 연결
- [ ] 베타 직전: 시민 업적 실제 날짜. 외계행성 운영 ON 여부. Daily Issue 자동운영 ON 여부. 실글 후 인기글/진영전투 브라우저 검증. 가입→게시판→반응→신고→탈퇴 실점검. 한국 서버 이전 후 로그인 속도 재측정

## ✅ 2026-08-30 — 게스트 읽기 전용

- [x] 게스트 좋아요·싫어요·공감 로컬 조작 차단. 서버 요청 없음
- [x] 글쓰기·댓글·팔로우·북마크·일반 신고도 사전 차단. 로그인 안내
- [x] 로그인 회원 기존 기능 경로 유지
- [ ] Railway 코드 배포 (이번 작업 아님)

## ✅ 2026-08-30 — 공식 게시글 배지 위조 방지

- [x] 공식 여부를 제목/본문 문자열이 아니라 `is_official` / `isOfficial`로 판단
- [x] 일반 회원은 플래그 조작·`[공식]` 제목 접두사로 공식 위조 불가
- [x] 공식 등록 도구만 true. 기존 글 백필 없음
- [x] 목록/상세 배지는 서버 `isOfficial === true`일 때만
- [ ] Production에 `migration_board_posts_is_official_v1.sql` apply (이번 작업 아님)
- [ ] 서버 이전 후 공식 9편 `--apply` 등록 (이번 작업 아님)

## ✅ 2026-08-30 — 베타 시작용 공식 기본 게시글

- [x] 안내 6편 + 토론 3편 콘텐츠 준비. 중앙광장. 가짜 회원/반응 없음
- [x] `[공식]` 표시. 고정글/공지 CMS/공식 계정 신설 없음
- [x] 중복 제목 건너뜀. Production 미등록
- [ ] 서버 이전 후 기존 운영자 UUID로 `--apply --confirm-dev-db --author-user-id` 등록 (이번 작업 아님)

## ✅ 2026-08-30 — 신규 회원 첫 방문 안내 + 상세 도움말

- [x] 가입 완료 직후 3단계 안내. 마지막은 중앙광장 게시판 진입
- [x] 회원당 최초 1회. 기존 가입완료 회원은 자동 노출 없음
- [x] 중앙광장 도착 보조 안내 1회
- [x] 성장·영토 안내에 이용방법·영토·반응·레벨/명성/업적·신고 정리. 성향 계산식/수치 비공개
- [x] 「영토 선택으로」→「영토 지도로」, 게스트 문구 정리
- [x] SQL additive 준비. Production apply · Railway 배포 안 함
- [ ] Production에 `migration_first_visit_guide_v1.sql` apply (이번 작업 아님)
- [ ] Railway 코드 배포 (이번 작업 아님)

## ✅ 2026-08-30 — 권리침해 신고 강화

- [x] 회원 필수 소명·증빙파일. 접수만으로 자동 제재 없음
- [x] 비회원 접수 완료 차단 (이메일 발송 미연결). 우회 경로 없음
- [x] 운영자 반려 코드·안내/내부 메모 분리. 확정 악용만 누적
- [x] 첨부 비공개 저장 SQL (Production 미적용)
- [x] 검토용 방침·운영정책 초안을 구현 범위만큼 갱신
- [ ] Production SQL apply · Railway 배포 (이번 작업 아님)
- [ ] 비회원 이메일 발송 수단 연결 후 비회원 접수 재개방

## ✅ 2026-08-30 — 법적·운영 문서 검토용 초안

- [x] 이용약관·개인정보처리방침·운영정책 초안 (`docs/legal/`)
- [x] 베타 전 확인 목록 (`docs/legal/BETA_LEGAL_OPEN_ITEMS.md`)
- [ ] 사업자·담당자·서버 국가·OAuth 실항목 확정 후 문구 채움
- [x] 권리침해 강화 구현 범위만큼 초안 수정
- [ ] 사이트 페이지 연결 (이용약관 페이지는 이번 작업 아님)

## ✅ 2026-08-30 — 가입 시 정치 정보 처리 + 현재 영토 공개 필수 동의

- [x] 가입 필수: 정치 관련 민감정보 처리 + 현재 소속 영토 공개. 미동의 시 가입 완료 불가
- [x] 가입 화면에서 정치성향 공개/비공개 선택 제거. 영토 공개는 회원 기능 필수 조건으로 표시
- [x] `political_profile_visibility` 컬럼 유지. 가입 POST로는 변경하지 않음
- [x] 서버 `POST /api/me/legal/sensitive-consent`에 영토 공개 동의 검증
- [x] additive 컬럼 `territory_disclosure_consented_at` / `territory_disclosure_policy_version` SQL만 추가
- [x] 기존 회원은 로그인 유지. 영토 공개 기록 없으면 로그인 후 동의 화면
- [x] 원점수·변화량·세부 계산·과거 이동 세부기록 비공개 유지
- [x] Production에 `migration_legal_territory_disclosure_v1.sql` apply (기존 행 자동 동의 없음)
- [ ] 코드 배포는 위 마이그레이션 적용 이후 (컬럼 SELECT/UPDATE가 필요)

## ✅ 2026-08-28 — Daily Issue 승인대기 운영

- [x] 아침 자동생성은 승인대기만. 운영자 승인 없이 공개 금지
- [x] 7일 승인대기 · 만료 후 30일 내부 보관 · issueDate 유지
- [x] 직접 수정 / AI 수정 요청 / 즉시·예약 재취합 / 버전 비교 / 폐기
- [x] 예약은 JSON/SQL persist. 재시작 후 복구. 중복 실행 방지
- [x] 공개글 업데이트 초안 → 승인 후 `최종 업데이트` 표시
- [x] 기존 수동 승인≠게시 API 유지. 새 `승인 및 공개` 경로 추가
- [x] Production 아침 스케줄러는 켜지 않음
- [x] `node tools/test-daily-issue-ops-workflow.js`
...
- [x] 임시 hotScore(공감×8·댓글개수×10·본문길이) 운영 인기글에서 제거
- [x] 서버 공식: 원글 공감×2 + 원글 LIKE + 원글 DISLIKE + 고유 댓글 참여자. 기간은 활동 시각
- [x] 일간(KST 오늘 0시) / 주간(7일) / 월간(30일) 탭. 작성일 제한 없음
- [x] 중앙광장·개척/수호 인기글 통일. ALIEN 신규 추가 없음. Guest 가짜 점수 미혼합
- [x] 정치성향/전황/명성 미변경. `node tools/test-popular-posts.js`
...
- [x] 인기글 실제 반응 순위 연동 (일간/주간/월간, 서버 집계)
- [ ] 실시간 영토 현황 실제 데이터 연동
