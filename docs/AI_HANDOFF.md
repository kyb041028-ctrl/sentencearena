...
1. 신고 검토(`reviewBehavior`)에서 숨김/제재 시 `admin_moderation_audit_events.report_id` 연결. 직접 조치는 NULL
2. 숨김 audit action은 `POST_SOFT_DELETE`/`COMMENT_SOFT_DELETE`. 실제 신고 숨김 상태는 기존 `HIDDEN_BY_OPERATOR` 유지
3. 복수 신고 → 대표 report 1건만 연결. 기각 audit는 `REPORT_REJECTED_AUDIT_NEEDED` 보류
4. `/admin/moderation/` 「이 신고의 처리 이력」→ `GET /api/admin/audit?reportId=`
5. 새 migration 없음. 테스트: `node tools/test-report-audit-link.js`
...
5. 보류 유지: retention purge, 제재 atomicity. 신고 report_id 연결은 2026-09-06 완료
...
5. 보류 유지: 보존기간 `ADMIN_AUDIT_RETENTION_POLICY_PENDING`. 제재 atomicity `SANCTION_AUDIT_ATOMICITY_LIMITATION`. 신고 report_id 연결은 2026-09-06 완료
6. 테스트: `node tools/test-admin-moderation-audit.js`

### [checkpoint] SESSION CLOSE DEFERRED (2026-08-30)

현재 Git: `master` = `origin/master` = `b2d3682` `feat: make guest mode read only`. 게스트 읽기 전용까지 코드 완료·push 완료. 오늘 추가 개발사항은 Production / Railway / Supabase에 운영 반영하지 않음.

1. 네이버 로그인: 개발자센터 재검수 진행 중. 결과 대기. 승인/반려에 따라 대응
2. 한국 서버 이전: 보류. 현재 Railway 유지. 추후 NAVER Cloud 한국 서버. 새 서버 완전 준비 후 DNS. Railway는 되돌리기용으로 잠시 유지. 한 번에 하지 말고 단계별
3. 가입 정치정보/영토공개 동의: 코드·테스트 완료. Production 동의 저장 칸 2개 apply 완료. 새 코드 운영 반영은 한국 서버 이전 후. 기존 회원 새 동의 화면·신규 Google/Kakao/Naver 가입 실제 확인 필요
4. 권리침해 강화: 회원 증빙·소명·반려·허위신고 코드 완료. Production SQL 미적용 → 한국 서버 이전 후. 브라우저 접수→첨부→운영자 검토→반려/처리 확인. 비회원 일반 신고 불가. 비회원 권리침해는 본인확인 수단 준비 전 실접수 차단. 이메일/본인확인 연결 보류. 정식 건 5년 보관 적정성 최종 검토
5. 첫 방문 안내: 3단계 코드 완료. Production SQL 미적용 → 한국 서버 이전 후. 신규 소셜가입 최초 1회·완료 후 중앙광장 진입·모바일 실화면 확인
6. 공식 게시글: 9편 준비 + `is_official` 위조방지 완료. Production SQL 미적용 → 한국 서버 이전 후 저장구조 적용. 기존 운영자 UUID로 9편 등록. 가짜 회원/댓글/반응 금지
7. 게스트: 읽기 전용 코드 완료(좋아요/싫어요/공감/글/댓글/답글/팔로우/북마크/일반신고/채팅 차단). 한국 서버 이전 후 브라우저 최종 확인
8. Daily Issue: 승인대기/버전/재취합 코드 완료. 실제 AI 연결 보류(현재 AI 수정 요청은 진짜 AI 아님). 아침 04:30 자동생성 ON 보류. Production 저장구조 최종 확인. 운영자 승인 전 자동공개 금지 유지
9. 법적 문서: 약관/개인정보/운영정책 초안 완료. 사이트 최종 연결 아님. 한국 서버 이전 후 서버/국외처리 확인, 사업자명·대표자·주소·문의메일·개인정보 담당자, 시행일, 권리침해 최종 구현 반영 후 연결
10. 베타 직전: 시민 업적 실제 베타 날짜. 외계행성 운영 최종 ON 여부. Daily Issue 자동운영 ON 여부. 실제 글 생긴 뒤 인기글/게시글별 진영전투 브라우저 검증. 가입→게시판→반응→신고→탈퇴 실브라우저 점검. 한국 서버 이전 후 로그인 속도 재측정
11. 로컬에만 남은 정치성향 시뮬 실험파일·`tools/verify-daily-issue-alignment-seed-live.js`는 커밋/삭제하지 말고 유지

### [checkpoint] GUEST READ-ONLY (2026-08-30)

1. 게스트는 열람만. 글쓰기·댓글·좋아요/싫어요/공감·팔로우·북마크·일반 신고·채팅 전송은 화면에서 사전 차단
2. 클릭 시 `회원가입 또는 로그인 후 이용할 수 있습니다.` 후 기존 `showLoginOnly` 로그인 화면
3. 서버 요청·로컬 숫자/선택 상태 변경 없음. 회원 canonical 경로 유지
4. auth.js / 성향 계산 / 명성·업적 미변경. Production/Railway/커밋 없음
5. 테스트: `node tools/test-guest-readonly.js`
...
### [checkpoint] MISINFO REPORT EXTRAS (2026-08-22)

1. 일반 신고 사유 목록 유지(`abuse|spam|baiting|misinfo|privacy|other`). misinfo 선택 시에만 정확한 표현·허위 이유·객관적 근거 필수
...
허위정보 범위: "허위정보 신고는 확인 가능한 구체적인 사실 주장에 한한다. 정확한 문제 표현과 객관적 근거가 필요하다."

제외: "의견, 평가, 예측, 추정, 풍자, 패러디는 원칙적으로 허위정보 신고 대상이 아니다. 사소한 오류만으로 허위정보로 확정하지 않는다."

접수 효과: "허위정보 신고 접수만으로 게시물을 삭제하거나 작성자를 제재하지 않는다. 운영자 확인이 필요하다."

남용: "악의적인 신고 남용이 확인되면 허위정보 신고 기능을 제한할 수 있다. 신고 반려만으로 신고자를 제재하지 않는다."

분리: "정치성향과 허위정보 제재는 분리한다."

---

### [checkpoint] RIGHTS EMAIL VERIFY (2026-08-22)

1. 비회원 권리침해 요청은 연락 이메일 일회용 인증 후에만 제출. 회원(로그인)은 추가 인증 없음. 기존 신청서 필수항목은 회원/비회원 모두 유지
2. 인증번호 6자리, 유효 10분(서버 시간), 원문 미저장(HMAC). 실패 5회 폐기. 같은 이메일 60초 재발송 금지. 메모리 rate limit 재사용. IP 장기보관/기기추적 없음
3. 제출 시 서버가 신청서 이메일과 인증 완료 이메일의 일치 확인. 이메일 변경 시 인증 무효
4. 인증 성공 ≠ 내용이 사실. 접수/정식전환/게시중단/계정 제재 자동 없음. 기존 중복방지·악용제재·보관정책 유지
5. 인증 임시기록은 단기 삭제. 사건 1년/5년 보관과 분리. 파일 첨부는 이번 작업 제외
6. 자동 테스트: `node tools/test-rights-email-verify.js` + `node tools/test-rights-infringement.js`
7. **STOPPED.** 기존 이메일 발송 코드/SMTP/발송 서비스/발신 주소 없음. 새 외부업체 미가입. Production commit/push/migration/deploy 하지 않음. 발송 수단이 준비되고 실제 수신 검증이 끝난 뒤에만 적용

#### 법적 문서용 확정 문구 (최종 약관 페이지 아님. 작성 시 구현 상태와 재대조)

비회원 이메일 확인: "비회원이 권리침해 처리 요청을 제출하는 경우 연락 가능한 이메일의 소유 여부를 확인하기 위해 일회용 인증 절차를 거칠 수 있다."

개인정보처리방침 작성 시 실제 구현과 대조할 항목: 비회원 연락 이메일, 이용 목적(권리침해 요청 연락 및 소유 확인), 보관기간(사건 기록은 기존 1년/5년, 일회용 인증정보는 만료 또는 확인 후 단기 삭제), 일회용 인증정보 처리(원문 미저장, 해시만 임시 보관).

---

### [checkpoint] RIGHTS INFRINGEMENT V1 (2026-08-22)

1. 권리침해 처리 요청은 일반 신고(`board_reports`)와 별도. 욕설/광고/분쟁유도는 기존 일반 신고. 명예훼손·개인정보·사진·저작권은 `/rights-infringement/`
2. 권리 당사자/대리인만 신청. 필수 구체정보 + 서버 검증(공백·짧은 문구 거부). 접수 ≠ 정식 사건. 자동 삭제/자동 제재 없음
3. 정치적 비판·평가 자체는 삭제 사유 아님. 구체적 사실 주장과 개인정보 공개는 별도 검토
4. 정식 사건만 최대 30일 임시 게시중단. 작성자 30일 이의제기. 신청자 이메일/증빙/내부 회원번호는 작성자에게 비공개
5. 악용은 운영자 확인 후에만: 경고 → 30일 제한 → 6개월 제한. 중대한 악용은 기존 제재 검토. 자동 영구정지 없음. 일반 신고는 막지 않음. 반려만으로 악용 제재 없음
6. 보관: 비정식 접수 최종 처리 후 1년. 정식 사건 최종 처리 후 5년. legal_hold 기존 규칙. 삭제 콘텐츠 6개월 증거는 운영자만 정식 사건에 연결
7. 1차 증빙은 설명+URL. 파일 업로드 인프라 없음. 비회원은 이메일 일회용 인증 후 제출(코드 준비, 발송 수단 없어 Production 미적용). IP 장기 보관/기기추적 없음
...
권리침해 분리: "명예훼손, 개인정보·사생활 침해, 사진·영상·초상, 저작권 등 권리침해 처리 요청은 일반 커뮤니티 신고와 별도의 절차로 운영한다."

당사자 절차: "권리침해 처리 요청은 권리 당사자 또는 정당한 대리인만 신청할 수 있으며, 대상 콘텐츠와 침해 이유를 구체적으로 제출해야 한다."

정치적 비판: "특정 정당·정치인·정부 비판, 정책 찬반, 정치적 가치판단과 강한 평가 자체는 권리침해 처리 사유가 아니다. 구체적인 사실 주장 및 개인정보 공개는 별도로 검토한다."

임시 게시중단: "정식 권리침해 사건으로 전환된 뒤 필요한 경우 해당 게시글 또는 댓글을 최대 30일간 임시로 게시 중단할 수 있다. 작성자는 30일 이내에 이의제기할 수 있다."

악용 제한: "허위 내용 제출, 자료 조작, 사칭, 반복 신청, 표적 신고 등 신고 제도 악용이 확인되면 권리침해 처리 요청 기능을 제한하거나 기존 이용제한 절차를 검토할 수 있다. 접수 반려만으로 제재하지 않는다."

보관 정식: "정식 권리침해 사건 기록은 최종 처리일로부터 5년간 보관한다."

보관 비정식: "정식 사건으로 전환되지 않고 보완 미완료 또는 반려된 접수 기록은 최종 처리일로부터 1년간 보관 후 파기한다."

삭제 증거: "이미 삭제된 게시물에 대한 정식 권리침해 사건은 운영자가 삭제 콘텐츠 보관 자료와 연결할 수 있다. 일반 사용자는 이 자료를 검색할 수 없다."

제재 분리: "권리침해 콘텐츠 처리 결과와 회원 계정 제재는 자동으로 연결하지 않는다. 중대한 운영정책 위반이 확인된 경우에만 기존 제재 절차를 검토한다."
...
5. 정식 권리침해 요청은 별도 시스템(`rights_infringement_*`). 접수는 정식 사건이 아님. 정식 사건만 최종 처리일+5년. 비정식 접수(보완 미완료/반려)는 최종 처리일+1년
...
7. **PRODUCTION PASS.** Railway `1cb7f30`. disposable만 제재 검증. 보호 계정 미제재. Alien V1 OFF
8. 자동 테스트: `node tools/test-user-sanctions.js`
