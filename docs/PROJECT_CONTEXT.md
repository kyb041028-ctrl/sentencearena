...
- **수집:** 교차 확인 가능 출처 묶음 + 공식기관 allowlist. 뉴스 사이트 대량 본문 크롤 금지.
- 검수·저장: repository · PostgreSQL · 관리자/공개 API · 관리자 UI(`/admin/daily-issues`). 개발 schema `daily_issue_test`. 운영 schema `daily_issue`(베타 전).
- **공개 UI:** 중앙광장 → `GET /api/daily-issues` · PUBLISHED·미만료만.
- **아침 스케줄러:** 04:30 수집 / 05:00 게시(KST) · 첫 Railway 배포 시 OFF 권장.
...
- **정치성향 (2026-08-16):** `CURRENT_TERRITORY_CANONICAL_SOURCE=profiles.territory` · **INITIAL_TERRITORY=CENTRAL** · **INITIAL_ALIGNMENT_SCORE=0** · **TERRITORY_SELECTION_UI=NOT_APPLICABLE** · **TERRITORY_SELF_WRITE=NOT_ALLOWED** · **BOARD_MEMBERSHIP_CONTEXT=PROFILES_TERRITORY** · **TERRITORY_MOVE=NOT_CONNECTED** · **TERRITORY_HISTORY=NOT_CONNECTED**. 사용자는 최초 영토를 고르지 않음. `POLITICAL_REACTION_INPUT=ACTIVE_CANONICAL` · **POLITICAL_SIMULATION=ACTIVE_READ_ONLY** · **CENTRAL_SIGN_POLICY=CONFIRMED** · **POLITICAL_SCORE_WRITE=ACTIVE_MANUAL** · **POLITICAL_BATCH_SCHEDULER=READY_DISABLED**. Guest `applyReactionScoresWithMult` 유지
