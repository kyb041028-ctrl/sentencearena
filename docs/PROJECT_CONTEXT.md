...
### 데일리 이슈 현재 운영 원칙 (2026-08-04)

- 데일리 이슈는 정치 성향 설문이 아닌 출처 기반 자유 토론 콘텐츠다.
- 답변 선택 기능은 제거되었고, 댓글/대댓글/반응은 선택 없이 사용한다.
- 데일리 이슈 열람·체류·댓글 작성 자체는 정치 성향 축을 이동시키지 않는다.
- 사용자 댓글·대댓글에 대한 커뮤니티 반응 집계/배치 규칙은 기존 그대로 유지한다.
- 이슈 게시는 정적 테마 풀 기반이라도 출처·문구 품질 게이트를 통과한 항목만 노출한다.
- RSS/API 수집, 실시간 교차검증, 가입 초기 성향 설문은 후속 과제다.
