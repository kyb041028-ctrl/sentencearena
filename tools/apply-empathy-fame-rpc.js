...
/**
 * apply_user_progression_event: EMPATHY_RECEIVED 는 XP 불변 · reputation only
...
const MIGRATION = path.join(__dirname, '..', 'supabase', 'migration_empathy_received_fame_rpc.sql');
...
async function main() {
  ...
  console.log(JSON.stringify({ ok: true, applied: 'migration_empathy_received_fame_rpc.sql', maskedUrl: maskDatabaseUrl(url) }));
}
